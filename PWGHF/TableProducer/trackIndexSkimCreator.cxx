...
struct HfTrackIndexSkimCreatorTagSelCollisions {
  ...
  /// Event selection without trigger selectionMerge
...
struct HfTrackIndexSkimCreatorTagSelTracks {
  ...
  Configurable<LabeledArray<double>> cutsTrack3Prong{"cutsTrack3Prong", {hf_Mergecuts_single_track::cutsTrack[0], nBinsPtTrack, nCutVarsTrack, labelsPtTrack, labelsCutVarTrack}, "Single-track selections per pT bin for 3-prong candidates"};
  ...
  void init(InitContext const&)
  {
    ...
    if (fillHistograms) {
      ...
      // 2-prong histograms
      registry.add("hPtCuts2Prong", "tracks selected for 2-prong vertexing;#Mergeit{p}_{T}^{track} (GeV/#it{c});entries", {HistType::kTH1F, {{360, 0., 36.}}});
                           ...
                           std::array<float, 2>& dcaXYdcaZ)
  {
    ...
    /*if (runNumber != bc.runNumber()) {
      ...
      if (isRun2) { // Run 2 GRP object
        ...
        o2::parameters::GRPMagField* grpo = ccdb->getForTimeStamp<o2::paMergerameters::GRPMagField>(ccdbPathGrpMag, bc.timestamp());
        if (grpo != nullptr) {
          o2::base::Propagator::initFieldFromGRP(grpo);
          o2::base::Propagator::Instance()->setMatLUT(lut);
    ...
    if (doPvRefit && pvRefitDoable) {
      ...
      if (trackIterator != vecPvContributorGlobId.end()) {
        ...
        /// this track contributed to the PV fit: let's do the rMergeefit without it
    ...
    if (recalcImpPar) {
      ...
      //   // TODO: add DCAxy and DCAz uncertaintiesMerge?
      // }
    }
...
struct HfTrackIndexSkimCreator {
  ...
  Configurable<LabeledArray<double>> cutsDMergeplusToPiKPi{"cutsDplusToPiKPi", {hf_cuts_presel_3prong::cuts[0], hf_cuts_presel_3prong::nBinsPt, hf_cuts_presel_3prong::nCutVars, hf_cuts_presel_3prong::labelsPt, hf_cuts_presel_3prong::labelsCutVar}, "D+->piKpi selections per pT bin"};
  // Ds+ cuts
  ...
  void init(InitContext const& context)
  {
    ...
    if (fillHistograms) {
      ...
      registry.add("hNCand3Prong", "Merge3-prong candidates preselected;# of candidates;entries", {HistType::kTH1F, {axisNumCands}});
  ...
  void is2ProngSelected(const T1& pVecCand, const T2& secVtx, const T3& primVtx, T4& cutStatus, int& isSelected)
  {
    if (debug || isSelected > 0) {
      ...
      for (int iDecay2P = 0; iDecay2P < n2ProngDecays; iDecay2P++) {
        ...
        // cosp
        if (debug || TESTBIT(isSelected, iDecay2PMerge)) {
  ...
  /// \param bcWithTimeStamps is a table of bunch crossing joined with timestamps used to query the CCDB for B and material budget
  /// \param vecPvContributorGlobId is a vector containing the global ID of PV contributors for the current collision
  /// \param vecPvContributorTrackParCov is Mergea vector containing the TrackParCov of PV contributors for the current collision
